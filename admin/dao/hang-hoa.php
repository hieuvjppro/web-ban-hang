<?php
    ...
    function hang_hoa_delete($ma_hh){//hàm xóa sp
        ...
        if(is_array($ma_hh)){
            foreach($ma_hh as $ma){ // vòng lặp foreach hiển thị theo &ma
                pdo_execute($sql,$ma);
            }
        }else{
    ...
    function hang_hoa_select_all_kemchongnang(){
        $sql = "SELECT * FROM hang_hoa JOIN loai_hang ON hang_hoa.ma_loai = loai_hang.ma_loai WHERE ten_loai='Kem Chống Nắng'";
        return pdo_query($sql);
    }

    function hang_hoa_select_all_kemduongam(){
        $sql = "SELECT * FROM hang_hoa JOIN loai_hang ON hang_hoa.ma_loai = loai_hang.ma_loai WHERE ten_loai='Kem Dưỡng Ẩm'";
        return pdo_query($sql);
    }
    ...
    function hang_hoa_ban_chay(){
        $sql = "SELECT * FROM hang_hoa ORDER BY don_gia DESC LIMIT 0,6"; // sắp xếp theo cột don_gia theo thứ tự giảm dần lấy 6 dòng tính từ thứ tự thứ 0
        return pdo_query($sql);
    }
